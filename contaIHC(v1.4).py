...
#       - v - in caso di tasti lettera, prevedi anche il blocco maiuscolo.
#       - v - impedisci di impostre due tasti uguali
# - o - salvataggio delle impostazioni su file esterno 
# - o - gestisci il tasto del mouse durante l'acquisizione conta
# - v - riproduci suono quando premuto un tasto sbagliato -> messaggio di errore
# - o - gestione delle sovrapposizione del Label risultati 
#
# 
# Per aprire il venv (virtual environment): 
#   cd env_tk_conta  
#   cd Scripts
#   activate 
#  
# Per compilare .exe sotto windows: 
#   pyinstaller contaIHC(v1.3).py --onefile --noconsole
...
def conta():
    ...
    while True: 
        ...
        keyboardTEMP.name = keyboardTEMP.name.lower()
        ...
        if keyboardTEMP.event_type == keyboard.KEY_DOWN and keyboardTEMP.name == tastoEND:
            ...
            pop.destroy()
            # text_risultato.destroy()
            risultato()
            window.update()
            break
        if keyboardTEMP.event_type == keyboard.KEY_DOWN:
            if keyboardTEMP.name != tastoA and keyboardTEMP.name != tastoB and keyboardTEMP.name != tastoEND:
                tastoEXTRA()
...
def tastoEXTRA():
    print("Tasto Extra")
    # beepy(sound="ping")
    messagebox.showerror("Errore!", "Hai premuto un tasto sbagliato!")
...
def set_A():
    ...
    if keyboardTEMP.name == tastoB or keyboardTEMP.name == tastoEND:
        messagebox.showerror("Errore!", "Tasto già utilizzato!")
    else:
...
def set_B():
    ...
    if keyboardTEMP.name == tastoA or keyboardTEMP.name == tastoEND:
        messagebox.showerror("Errore!", "Tasto già utilizzato!")
    else:
...
def set_END():
    ...
    if keyboardTEMP.name == tastoB or keyboardTEMP.name == tastoA:
        messagebox.showerror("Errore!", "Tasto già utilizzato!")
    else:
